...
class ASTHTMLElement:
    ...
    __element_template__ = """el("{}",[{}],oneline={},{})"""
    ...
    def python(self) -> str:
        ...
        if self.name[0].capitalize() == self.name[0]:
            ...
            result = self.__element_template__.format(self.name, ",".join(child.python() for child in self.children), self.one_line, attributes)
